...
-- 단 일 행 함수 : N개의 값을 읽어서 연산후 N 개의 결과를 반환
-- 그룹 함수    : N개의 값을 읽어서 연산후 1개의 결과를 반환 (합계 , 평균, 최대, 최소)


-- 함수는 SELECT 문의 
-- SELELCT 절, WEHRE절 , ORDER BY 절 , GROUP BY 절, HAVING 절 사용가능
...
SELECT EMAIL, LENGTH('가나다라마바사') -- 7개 
FROM EMPLOYEE;

--------------------------------------------------------------
-- INSTR 찾고자하는 문자를 검색할때
...
-- 문자열을 앞에서부터 검색하여 첫번째 B의 위치를 조회해라
...
-- EMPLOYEE 테이블에서 사원명 , 이메일 중 아이디만 조회 (@이전껏만)
...
-- MOD (숫자 | 컬러명 , 숫자| 컬럼명) : 나머지 값 반환  모듈러의 MOD 임

-- EMPLOYEE 테이블에서 사원의 월급을 100만으로 나눴을떄 나머지 조회

SELECT EMP_NAME, SALARY , MOD (SALARY , 1000000) 급여나누기100만 
FROM EMPLOYEE;
...
FROM EMPLOYEE 
WHERE MOD  (EMP_ID, 2) = 0; -- 홀수는 부정연산자만 붙여주면됨 ..

SELECT EMP_ID , EMP_NAME
FROM EMPLOYEE 
WHERE MOD  (EMP_ID, 2) != 0; --홀수
...
-- 123, 소수점 첫번째 자리에서 반올림
SELECT ROUND(123.456,1) FROM DUAL;  
--123,5 소수점 소수점 두번째 자리에서 반올림하여수점 첫번째 자리까지 표기하겠다 라는 의미

--CEIL(숫자 | 컬럼명 ) : 올림
--FOLLR (숫자 | 컬럼명) : 내림
SELECT CEIL(123.1), FLOOR (123.9) FROM DUAL;

-- TRUNC (숫자 | 컬러명 [,위치]) : 특정 위치 아래를 절삭

SELECT TRUNC ('123.456') FROM DUAL; -- 123 ,소수점 아래를 절삭

SELECT TRUNC ('123.456',1) FROM DUAL; --123.4 소수점 첫째자리 아래줄을 다 절삭

SELECT TRUNC ('123.456',-1) FROM DUAL; --120, 10의자리 아래를 절삭
...
SELECT SYSDATE FROM DUAL; --2025-03-10 12:11:02.000

--SYSTIMESTAMP : SYSDATE + MS 단위 추가 
SELECT SYSTIMESTAMP FROM DUAL; --2025-03-10 12:12:07.208 +0900
-- +0900 은 UTC 의 +9 시간 한것이다 UTC는 세계 표준시간 ?

-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월수 차이반환
SELECT ABS (ROUND(MONTHS_BETWEEN(SYSDATE,'2025-07-22'),3) ) "수강 기간(개월)"
FROM DUAL;
-- 4.371 : 차이의 개월수 소수점 3짜짜리까지 절대값으로 표현
...
CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE) / 12) || '년차' "근무년 차"
FROM EMPLOYEE;

-- || : 연결 연산자 (문자열 이어쓰기)

-- ADD_MONTHS (날짜 , 숫자) : 날짜에 숫자만 큼의 개월수를 더함(음수도 가능)
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL; -- 오늘 날짜에 4개월 더함
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL; -- 오늘 날짜에 1개월 뺌
SELECT ADD_MONTHS(SYSDATE, -5) FROM DUAL; -- 오늘날짜에 5개월 전

-- LAST_DAY (날짜) : 해당달의 마지막 날짜를 구함
SELECT LAST_DAY (SYSDATE) FROM DUAL; --2025-03-31 12:25:57.000 현재 달의 마지막날
SELECT LAST_DAY ('2020-02-01') FROM DUAL;--2020-02-29 00:00:00.000 윤달이엿다
...
-- 문자열로 변환
-- TO_CHAR(날자, [포맷]) : 날짜형 데이터를 문자형 데이터로 변경
-- TO_CHAR(숫자, [포맷]) : 숫자형 데이터를 문자형 데이터로 변경

-- [포맷]
-- 숫자 - > 문자 변환시 포맷 패턴
-- 9 : 숫자 한칸을 의미 ,여러 개작성시 오른쪽 정렬 
-- 0 : 숫자 한칸을 의미, 여러개 작성시 오른쪽 정렬 +빈칸 0으로 채워주는 역할
...
SELECT TO_CHAR (1000000, 'L9,999,999') || '원'FROM DUAL; -- '\1,000,000'
...
--> "" 쌍 따음표를 이용해서 단순한 문자로 인식시키면 해결됨.

--------------------------------------------------------------------------------

-- 날짜로 변환 TO_DATE
-- TO _ DATE(문자형 데이터,[포멧]) : 문자형 -> 날짜로 변경
-- TO _ DATE(숫자형 데이터,[포멧]) : 숫자형 - > 날짜로 변경
--> 지정된 포맷으로 날짜를 인식함
...
--> 패턴을 정용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인식 시킴
...
SELECT TO_DATE('800505', 'RRMMDD') FROM DUAL; --2030-05-05 00:00:00.000
SELECT TO_DATE('490505', 'RRMMDD') FROM DUAL; --2049-05-05 00:00:00.000


-- EMPLOYEE 테이블에서 각 직원이 태어난 생년월일 조회
-- 사원이름, 생년월일(1965년 10 월 08일)

SELECT * FROM EMPLOYEE;
-- EMP_ NO 621231-1985634
...
-- 숫자 형변환
--TO_NUMBER (문자데이터, [포맷]) : 문자형 데이터를 숫자 데이터 변경

SELECT '1,000,000' + 500000 FROM DUAL;
--QL Error [1722] [42000]: ORA-01722: 수치가 부적합합니다

SELECT TO_NUMBER('1,000,000','9,999,999') + 500000 FROM DUAL;
-- 숫자 안에(이게 문자형을 -> 숫자형으로 바꾼것) + 500000 한것
-- 1,500,000

-- NULL 처리 함수
-- NVL(컬럼명, 컬럼값이 NULL 일때 바꿀 값) : NULL 인 컬럼 값을 다른값으로 변경
...
-- EMPLOYEE 테이블에서
-- 급여가 500만원 이상이면 '대'
-- 급여가 300만원 이상 500만원 이하면 '중'
-- 급여가 300만원 미만이면 '소' 로조회
-- 사원이름, 급여, 급여 받는정도 조회
...
-- 그룹 함수
-- 하나 인상의 행을 그룹으로 묶어 연산하여 총합, 평균 등의 
-- 하나의 결과 행으로 반환하는 함수
...
/*3*/SELECT SUM(SALARY) "급여 합" , ROUND(AVG (SALARY)) "급여 평균"
/*1*/FROM EMPLOYEE
/*2*/WHERE DEPT_CODE = 'D9'; -- 급여합17700000 , 급여 평균5900000
