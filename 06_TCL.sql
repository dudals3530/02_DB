...
--> DB 에 반영된것 처럼 보이지만
-- 실제로 아직 DB에 영구반영된것 아님
-- 트랜잭션에 INSERT 3개 있음

-- ROLLBACK 후 확인
ROLLBACK; -- 마지막 커밋 시점 까지 되돌아감.

SELECT * FROM DEPARTMENT2;
-- 개발1, 개발2,개발3 팀 증발...ㅜㅜ

-- COMMT 후 ROLLBACK 이 되는지 확인
INSERT INTO DEPARTMENT2 
VALUES('T1','개발1팀','L2');
INSERT INTO DEPARTMENT2 
VALUES('T2','개발2팀','L2');
...
ROLLBACK TO "SP1"; -- SP1 이후꺼는 없어지고 SP1 이전꺼까지는 살아있음.
                   -- 즉 개발4팀은 남아있음

ROLLBACK TO "SP2";
--SQL Error [1086] [72000]: ORA-01086: 'SP2' 저장점이 이 세션에 설정되지 않았거나 부적합합니다. 
-- 이전에 ROLLBACK TO "SP1"; 함으로써 SP2,SP3 도 같이 TSA에 삭제됨. (TSA = TRANSACTION)
...
SELECT * FROM DEPARTMENT2;
-- 개발 6팀만 없음(SP2지점 SAVEPOINT 전 개발5팀, 개발4팀 있음)
