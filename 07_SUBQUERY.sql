...
--3)  부서코드가 노옹철 사원과 같은 소속의 직원 명단조회
--> 위의 2개의 단계를 하나의 쿼리로!!
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = (
 ...
 * 
 * - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 나열된 항목수가 여러개일 때 (단, 행은1개)
 * 
...
--1) 부서별 급여 합중 가장 큰값 조회(서브쿼리)
SELECT MAX(SUM(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 17,700,000

-- 부서별급여합이 17,700,000 인 부서의 부서명과 급여합 조회
SELECT DEPARTMENT.DEPT_TITLE , SUM(SALARY)
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE=DEPT_ID)
GROUP BY DEPT_TITLE 
HAVING SUM(SALARY) = 17700000; 

--3) 위의 두 쿼리를 합쳐 부서별 급여 합이 가장 큰부서의
--부서명, 급여합 조회
...
-- 2. 다중행 서브 쿼리 (MULTI ROW SUBQUERY)
-- 서브쿼리의 조회 결과 값의 개수가 여러 행일때

/*
 *  >> 다중행 서브 쿼리 앞에는 일반 비교연산자 사용 불가
 *
 * - IN/ NOT IN : 여러개의 결과 값 중에서 한개라도 일치하는 값이 있따면 
 *               혹은 없다면 ( 가장 많이 사용 ! )
 * 
 * - > ANY , < ANY : 여러개의 결과값 중에서 한개라도 큰 / 작은 경우
 *              --> 가장 작은값보다 큰가? / 가장 큰 값보다 작은가 ?
 * 
 * -  > ALL, < ALL : 여러개의 결과값의 모든 값보다 큰/ 작은경우
 *              --> 가장 큰값보다 큰가 ?가장 작은값보다 작은가 ?
 * 
...
FROM EMPLOYEE
WHERE MANAGER_ID IS NOT NULL;  -- 이게 누군가의 사수 번호
...
--4) 일반 직원에 해당 하는 사원들 정보조회 (구분은 '사원')
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME,
       DECODE(
           (SELECT COUNT(*) 
            FROM EMPLOYEE E
            WHERE E.MANAGER_ID = EMPLOYEE.EMP_ID), 
           0, '사원', 
           '사수'
       ) 구분
FROM EMPLOYEE
...
AND SALARY > ANY (SELECT SALARY
 ...
 --       > ANY : 가장 작은값 보다 큰가 ?
-- 우리문제에서는 결국 >> 과장직원 가장작은 급여값 보다 큰 급여를 받는 대리가 있는가?

--        < ANY : 가장 큰값보다 작은가 ?
-- 우리문제에서는 결국 >> 과장직원의 가장큰 급여값보다 작은 급여를 받는 대리가 있는가 ?
...
--    > ALL : 가장 큰갑보다 큰가 ?
-- 즉 차장의 MAX급여값보다 큰 급여를 받는 과장이 있는가? 

--    < ALL : 가장 작은갑보다 작냐?
-- 즉 차장의 MIN급여값보다 작은 급여를 받는 과장이 있는가?
...
-- LOCATION 테이블에서 NATIONAL_CDOE 가 KO인 경우의 LCAL_CODE
SELECT LOCAL_CODE
FROM LOCATION 
WHERE LOCATION.NATIONAL_CODE = 'KO'; -- L1 -> 단일행 서브쿼리

--DEPARTMENT 테이블의 LOCATION_ID 와 동일한 DEPT_ID
SELECT DEPT_ID
FROM DEPARTMENT
WHERE LOCATION_ID =(SELECT LOCAL_CODE
                    FROM LOCATION 
                    WHERE LOCATION.NATIONAL_CODE = 'KO'); --D1,D2,D3,D4,D9 -다중행 서브쿼리

-- 최종적으로 EMPLOYEE 테이블의 DEPT_CODE 와 동일한 사원 조회히라.
SELECT EMP_NAME,DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID
                    ...
                    WHERE LOCATION_ID =( SELECT LOCAL_CODE
                                         FROM LOCATION 
                                         WHERE LOCATION.NATIONAL_CODE = 'KO'));
...
AND ENT_YN = 'Y';  -- D8,J6 (이름이 이태림이래..) 

-- 퇴사한 여직원과 같은부서, 같은직급 조회

-- 방법 1 )단일 행 단일열 서브쿼리 2개를 사용해서 조회
SELECT DEPT_CODE 
FROM EMPLOYEE
WHERE ENT_YN = 'Y'
...
-- 2000년도에 입사한 사원의 부서와 직급
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '2000-01-01' AND ' 2000-12-31';  -- D6,J3
...
--1) 직급별 평균 급여   --> 서브쿼리
SELECT JOB_CODE, TRUNC(AVG(SALARY),-4) "직급별평균급여"
FROM EMPLOYEE
GROUP BY JOB_CODE;
...
SELECT EMP_ID ,EMP_NAME,JOB_CODE , SALARY
FROM EMPLOYEE
WHERE (JOB_CODE, SALARY) IN (SELECT JOB_CODE, TRUNC(AVG(SALARY),-4) "직급별평균급여"
                             FROM EMPLOYEE
                             GROUP BY JOB_CODE);
...
SELECT EMP_ID, EM
P_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음'),
JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
