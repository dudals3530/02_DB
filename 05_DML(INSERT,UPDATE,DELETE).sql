-- ** DML(Data Manipulation Language) : EPDLXJ WHWKR DJSDJ **

-- 테이블에 값을 삽입하거나 (INSERT), 수정하거나(UPDATE), 삭제 (DELETE)하는 구문

-- 주의 : 혼자서 COMMIT, ROLLBACK 수행 하지 말것!
-- 윈도우 > 설정 > 연결 > 연결 유형 AUTO COMMIT BY DEFAULT 체크 해제 되어있는지 다시 확인!

-- 테스트용  테이블 생성
CREATE TABLE EMPLOYEE2 AS SELECT * FROM EMPLOYEE;
CREATE TABLE DEPARTMENT2 AS SELECT * FROM DEPARTMENT;
-- DDL 수정시 자동으로 커밋됨   즉 , DML, DDL 섞어서 쓰면절대 안됌.
-- 테이블 만들시 이건 DDL 이라 자동 커밋됨 . ㅇㅇ.. 글쿤 
-- DDL 이 DATA DEFINATION LANGUAGE 아마 이거일꺼임 ..
-- 데이터를 정의 하는 의미 ? 
...
-- 테이블에 있는 모든 컬럼에대한 값을 INSERT 할 때사용
-- 단, 컬럼의 순서를 지켜서 VALUES에 값을 기입해야함.

INSERT INTO EMPLOYEE2 VALUES('900','홍길동','123456-7894563','hong_gd@or.kr',
...
VALUES ('900','홍길동','123456-1234567','DUDALS3530@OR.KR','01012345678','D1','J7','S3',43000000);
...
ROLLBACK; -- 롤백을 수행했으나 롤백이안됨. 데이터를 영구저장 햇기에안되는거임
-- 데이터가 TRANSATION 에 있던걸 COMMIT 하는순간 데이터 영구저장됨 .
SELECT * FROM EMPLOYEE2 
WHERE EMP_ID = '900';
------------------------------------------------------------------------------------------------
...
-- DEPARTMENT2 테이블에서 DEPT_ID가 'D9' 인부서 정보조회

SELECT * 
FROM DEPARTMENT2
WHERE DEPT_ID ='D9';

--DEPARTMENT2 테이블에서 DEPT_ID가 'D9' 인부서 
--DEPT_TITLE을 '전력 기획팀' 으로수정
...
-- *여러 컬럼을 한번에 수정할시 콤마(,)로 컬럼을 구분하면됨.
-- D9 / 총무부 -- > D0 /전략기획팀변경
...
-- 기존에있던 행에는 MERGE를 이용하면 UPDATE 가 되고
-- 기존에없던 행을 MERGE 하게되면 INSERT 가 되는 거네
...
INSERT INTO EMP_M02
VALUES(999,'곽두원','567016-1011111','DUDALS353@OR.KR',
'010202222222','D9','J4','S1',9000000,0.5 ,NULL,SYSDATE,NULL,'N');

SELECT * FROM EMP_M01; -- 23명
SELECT * FROM EMP_M02; -- 5명 (기존 4+신규1)
...
-- 롤백후 복구확인
SELECT * FROM EMPLOYEE3; -- 롤백을 했지만 복구 안됨. 즉 DDL 이라 TRANSACTION 을 안통하고 바로 삭제됨.
