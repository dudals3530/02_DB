...
-- 여러개의 결과값을 산출하기 위해 그룹함수가 적용된 그룹의 기준을 ORDER BY 절에 기술하여사용
...
-- 직급코드가 같은사람의(직급을 그룹으로 묶기 : JOB_CODE)
-- 직급코드, 급여평균, 인원 수를 (SELECT 절)
...
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여' ) --별칭 사용x (SELECT 절 해석 x)
...
-- EMPLOYEE 테이블 에서 (FROM 절)
-- 부서코드가 D5, D6인 부서의(WHERE 절 -> GROUPBY 절)
...
-- ** GROUP BY 사용시 주의사항 **
--> SELECT 문에 GROUP BY 절을 사용 하는 경우
-- SELECT절에 명시한 조회하려는 컬럼중
-- 그룹함수가 적용되지 않은 컬럼을
-- 모두 GROUPY BY 절에 작성되어있어야함.

-- EMPLOYEE 테이블에서 (FROM 절) 
-- 부서별로 같은 직급인 사원의 인원수를 조회 (GROUP BY절 DEPT_CODE,JOB_CODE 묶음)
-- 부서코드 , 오름차순, 직긐코드 내림차순으로 정렬 (ORDER BY 절)
-- 부서코드, 직급코드,인원수(SELECT 절)

SELECT DEPT_CODE,JOB_CODE,COUNT(*)
FROM EMPLOYEE
GROUP BY DEPT_CODE ,JOB_CODE            -- DEPT_CODE 그룹을 나누고,
...
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹중 급여 평균이 3백만원 이상인 그룹만 조회
ORDER BY DEPT_CODE ;
...
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING COUNT (*) <= '5'   -- HAVING 절에는 그룹함수가 반드시 작스엉!
ORDER BY JOB_CODE ;

---------------------------------------------------------------------------

-- 집계 함수 (ROLLUP, CUBE)
-- 그룹 별로 산출 결과 값의 집계를 계산하는 함수
--(그룹별로 중간집계결과를 추가)  ?  ?이게뭔말이지 . . . .
-- GROUP BY 절에서만 사용가능

--ROLLUP : GROUP BY 절에서 가장 먼저 작성된 컬럼의 중간 집계를 처리하는 함수
-- 중간집계가 뭔ㅁ라잊.. .흠 . .감이안온다 .. 
-- NULL 값이 나오는데 이게 첫번쨰 GROUP BY 의 총갯수를 나타내는 약간 정산같은느낌

SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY ROLLUP (DEPT_CODE, JOB_CODE) 
ORDER BY 1;   -- 1이 DEPT_CODE를 오름  차순 한다는 소리임

-- CUBE : GROUPY BY 절에 작성된 모든 컬럼의 중간 집계를 처리하는 함수
...
-- CUBE 는 GROUP BY 에서 결국 마지막 컬럼에 작성한 총 갯수를 집계하는 처리함수.
 ...
 * 
 * - UNION ALL : UNION + INTERSECT 합집합에서 중복부분 제거를 안해줌
 * 
 * - MINUS(차집합) : A에서 A,B 교집합 부부은 제거하고 조회
 * 
 * */

-- EMPLOYEE 테이블에서
--(1번째 SELECT) 부서코드가 'D5'인 사원의 사번, 이름 ,부서코드, 급여 조회
