...
ORDER BY 사번;
-- 기본의 객체가 이름을 사용 하고 있습니다.

SELECT * FROM V_EMP;

-- V_EMP 에서 대리 직원들을 이름 오름차순 으로 조회
-- VIEW 조회 결과로 보이는 컬럼명을 이용해야한다!
-- 약간 음.. 교육용 ? , 연수용 ? 이런걸로 만들면 좋아보이긴하네
SELECT * FROM V_EMP
WHERE 직급명 = '대리'
ORDER BY 이름;
...
-- 왜 사용 할까 ?
--> VIEW 를 이용 해서 DML(INSERT/UPDATE/DELETE) 막기위해서 !!
...
INSERT INTO V_DCOPY2
VALUES('D0','L3');
--42399: 읽기 전용 뷰에서는 DML 작업을 수행할 수 없습니다.
 ...
 *   [작성법]
  ...
  [CACHE 시퀀스개수 | NOCACHE] -- 기본값은 20                       --> 약간 사전쿠폰같은느낌
...
INCREMENT BY 5 -- NEXTVAL 하면 5씩증가 , 감소도가능하지만 잘안함
MAXVALUE 150    -- 증가 가능한 최대값 150
NOMINVALUE      -- 최소값없음 
NOCYCLE         -- 반복 안함 
NOCACHE;         -- 미리만들어둘 시퀀스 번호없음 

-- 시퀀스생성잘됨 RESCOURCE 권한 안에 시퀀스생성도잇어서그럼 ..
...
--> CURRVAL의 정확한 의미는
-- 가장최근에 호출된 NEXTVAL 의 값을 반환함을 뜻함
--> NEXTVAL 을 호출한적이없어서 오류발생!
...
--TB _TEST 테이블에 PK값을 SEQ_TEST_NO 시퀀스 로 생성하기
INSERT INTO TB_TEST
...
-- UPDATE 에서 시퀀스 사용하기
-- '짱구'의 PK 컬럼값을 
-- SEQ_TEST_NO 시퀀스의 다음 생성값을 변경하기


UPDATE TB_TEST
SET TEST_NO  = SEQ_TEST_NO.NEXTVAL
WHERE TEST_NAME = '짱구';
--SEQ_TEST_NO.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다
-- > MAXVALUE 150 보다 더 증가할수없다.
SELECT * FROM TB_TEST;
...
/*[작성법]
 * ALTER SEQUNECT 시퀀스 이름
 * [INCREMENT BY 숫자]
 * [MAXVALUE 숫자 | NOMAXVALUE]
 * [NOMINVALUE | MINVALUE 숫자]
 * [CYCLE | NOCYCLE]
 * [CACHE 숫자 | NOCACHE]
 * */
...
FROM EMPLOYEE;
-- ROWID : 오라클에서 각행의 고유한 주소를 나타내는 가상 컬럼 (물리적 주소)
-- 인덱스가 ROWID 저장함.
...
-- EMPLOYEE 테이블에 EMP_NAME 컬럼에 인덱스 생성
CREATE INDEX IDX_EMP_NAME
ON EMPLOYEE(EMP_NAME);
--> EMPLOYEE 테이블에 EMP_NAME 컬럼에 대해 빠른검색이 가능
...
CREATE TABLE TB_IDX_TEST (
	TEST_NO NUMBER PRIMARY KEY, -- UNIQUE INDEX 생성됨
...
-- *PL/SQL 이란 ?
-- 오라클 데이터베이스에서 사용하는 절차적 확장 언어. ?????? 이게뭔말이야 ..
...
WHERE TEST_ID = 'TEST500000'; --0.019S


/* 인덱스 O*/ 
-- TEST_NO 가 500000인 행을 조회하기
SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 500000; --0.000S 개빠르다 .. 
